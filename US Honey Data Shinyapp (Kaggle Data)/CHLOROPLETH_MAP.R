...
honey_data$mean_totalprod <- 0

enterST <- function(x){
  return (mean(honey_data$totalprod[which(honey_data$state == x)]));
}

for(i in 1:50){
  x = state.abb[i]
  honey_data$mean_totalprod[which(honey_data$state == x)] <- enterST(x)
}

state_tb <- as.data.frame(honey_data)
...
ggplot(data=states.dat,aes(x=long.x, y=lat.x)) + 
